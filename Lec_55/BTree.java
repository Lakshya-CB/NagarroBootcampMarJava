...
//		Do remaing work!! 
	...
	public void vertical_order() {
		HashMap<Integer, ArrayList<Integer>> map = new HashMap<>();

		Queue<pairx> Q = new LinkedList<>();
		Q.add(new pairx(root, 0));
		while (!Q.isEmpty()) {
			pairx pp = Q.poll();
			...
			if (!map.containsKey(pp.x)) {
				ArrayList<Integer> Curr_x = new ArrayList<>();
				Curr_x.add(pp.n.data);
				map.put(pp.x, Curr_x);

			} else {
				ArrayList<Integer> Curr_x = map.get(pp.x);
				Curr_x.add(pp.n.data);
			}

//			System.out.println(pp.x + "@" + pp.n.data);
			Q.add(new pairx(pp.n.left, pp.x - 1));
			Q.add(new pairx(pp.n.right, pp.x + 1));
		}
